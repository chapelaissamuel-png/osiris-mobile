...
const LAYER_GROUPS = [
  ...
  },
  {
    ...
    layers: [
      ...
      { key: 'planetarium', label: 'Planétarium (ciel réel)', icon: Star, color: '#5fd0e6', dataKey: '' },
    ],
  },
];
                    ...
                    <div className="flex flex-col gap-1.5">
                      {group.layers.map((layer) => {
                        ...
                        return (
                          ...
                          {layer.key === 'planetarium' && isLayerActive && (
                            <div className="flex items-center gap-2 px-2 pb-1">
                              <span className="text-[9px] font-mono text-white/40 w-12">OPACITY</span>
                              <input
                                type="range" min="10" max="100" step="5"
                                value={activeLayers.planetarium_opacity ?? 80}
                                onChange={e => setActiveLayers((prev: any) => ({ ...prev, planetarium_opacity: Number(e.target.value) }))}
                                className="flex-1 h-1 cursor-pointer"
                                style={{ accentColor: '#5fd0e6' }}
                              />
                              <span className="text-[9px] font-mono text-white/40 w-8 text-right">{activeLayers.planetarium_opacity ?? 80}%</span>
                            </div>
                          )}
                          </div>
                        );
