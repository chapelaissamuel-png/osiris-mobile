 ...
 * Primary:     SatNOGS DB (open JSON TLE API)
 * Supplemental: CelesTrak active.tle (TLE text format, public)
 * Propagation: Simplified SGP4
 */
...
// ─── TLE text parser (CelesTrak format) ───────────────────────────────────────
function parseTleText(text: string): Array<{ name: string; line1: string; line2: string }> {
  ...
  for (let i = 0; i < lines.length - 2; i++) {
    ...
    sats.push({ name, line1: lines[i + 1], line2: lines[i + 2] });
    i += 2;
  }
  return sats;
}
...
// ─── In-memory cache ──────────────────────────────────────────────────────────
const globalForSat = globalThis as unknown as {
  satCache: any[];
  satCacheTime: number;
  satSource: string;
};
if (!globalForSat.satCache) {
  globalForSat.satCache   = [];
  globalForSat.satCacheTime = 0;
  globalForSat.satSource  = '';
}
...
export async function GET() {
  try {
    ...
    let allSats: any[] = globalForSat.satCache;
    let source = globalForSat.satSource || 'memory-cache';

    if (allSats.length === 0 || nowTime - globalForSat.satCacheTime > 3600000) {
      // Fetch SatNOGS and CelesTrak in parallel
      const [satnogsResult, celestrakResult] = await Promise.allSettled([
      ...
      // SatNOGS (JSON)
      if (satnogsResult.status === 'fulfilled') {
        for (const item of satnogsResult.value) {
          const rawName = (item.tle0 || '').trim();
          const cleanName = rawName.replace(/^0\s+/, '');
          if (cleanName && item.tle1 && item.tle2 && !seen.has(cleanName)) {
            seen.add(cleanName);
            fetched.push({ name: cleanName, line1: item.tle1.trim(), line2: item.tle2.trim() });
          }
        }
        if (fetched.length > 0) sources.push('SatNOGS');
      } else {
        console.error('[OSIRIS/Satellites] SatNOGS error:', satnogsResult.reason);
      }

      // CelesTrak (TLE text) — adds satellites not in SatNOGS
      if (celestrakResult.status === 'fulfilled') {
        const ctSats = parseTleText(celestrakResult.value);
        let ctAdded = 0;
        for (const s of ctSats) {
        ...
        if (ctAdded > 0) sources.push(`CelesTrak (+${ctAdded})`);
      } else {
        console.error('[OSIRIS/Satellites] CelesTrak error:', celestrakResult.reason);
      }

      if (fetched.length > 0) {
        globalForSat.satCache     = fetched;
        globalForSat.satCacheTime = nowTime;
        globalForSat.satSource    = sources.join(' + ');
        allSats = fetched;
        source  = globalForSat.satSource;
      }
    }
    ...
    const sampled = allSats.length > 2000
      ? allSats.filter((_: any, i: number) => i % Math.ceil(allSats.length / 2000) === 0)
      : allSats;
    ...
    console.error('[OSIRIS/Satellites] Fatal error:', error);
